...
HPC clusters typically do not have Git LFS pre-installed. Install it via conda and pull the binary model files:
...
**3. Build and install htm.core from source via SLURM**
...
**4. Place the UB dataset** so the repo and dataset share a parent directory:
...
**5. Adapt the SLURM scripts** for your cluster — edit the `##SBATCH` partition/account lines in each script you use (they are double-commented by default so SLURM ignores them):
